package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_451:
               ...
               {
                  case RoomSessionQueueEvent.const_902:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_894:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_978) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_978) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1322) + 1;
                  }
               }
               else
               {
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_698:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_894);
               break;
            case RoomWidgetRoomQueueMessage.const_651:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_902);
               break;
            case RoomWidgetRoomQueueMessage.const_505:
               if(false)
               {
      ...
      {
         return [RoomSessionQueueEvent.const_451];
      }
