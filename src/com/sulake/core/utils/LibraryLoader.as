package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected var var_347:Class;
      
      protected var var_2383:uint = 0;
      ...
      protected function prepareLibrary() : void
      {
         ...
         var_347 = this.getDefinition(_name) as Class;
         if(var_347 == null)
         {
         ...
         try
         {
            xmlClass = var_347.manifest as Class;
            if(xmlClass == null)
            {
               return;
            }
         ...
         var_499 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            case Event.INIT:
               ...
               var_448 |= LibraryLoader.STATE_EVENT_INIT;
               break;
            case Event.COMPLETE:
               debug("Load event COMPLETE for file \"" + var_15.contentLoaderInfo.url + "\"");
               var_448 |= LibraryLoader.STATE_EVENT_COMPLETE;
               break;
            case Event.ENTER_FRAME:
               break;
            case HTTPStatusEvent.HTTP_STATUS:
               var_370 = HTTPStatusEvent(param1).status;
               debug("Load event STATUS " + var_370 + " for file \"" + var_15.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
         ...
         if(var_448 == LibraryLoader.STATE_ANALYZE)
         {
            if(analyzeLibrary())
            {
               var_448 |= LibraryLoader.STATE_EVENT_FRAME;
            }
         }
         if(var_448 == LibraryLoader.STATE_READY)
         {
            var_1382 = true;
            prepareLibrary();
            removeEventListeners();
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_934 > 0)
            {
               var_15.close();
               var_15.unload();
               addRequestCounterToUrlRequest(var_322,const_1393 - var_934);
               var_15.load(var_322,var_14);
               --var_934;
               return true;
            }
