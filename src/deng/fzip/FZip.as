package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
         ...
         if(param1.bytesAvailable >= 4)
         {
            ...
            switch(_loc2_)
            {
               case 67324752:
                  ...
                  var_532 = new FZipFile(charEncoding);
                  break;
               case 33639248:
               case 101010256:
                  parseFunc = parseIdle;
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_527))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_527,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_92 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_527,"EOF"));
            }
         }
      }
      ...
      {
         if(var_532.parse(param1))
         {
            var_46.push(var_532);
            if(false)
            {
               var_138["null"] = var_532;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1161,var_532));
            var_532 = null;
