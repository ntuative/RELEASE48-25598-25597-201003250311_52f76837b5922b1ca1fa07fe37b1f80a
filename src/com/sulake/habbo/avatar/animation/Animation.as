package com.sulake.habbo.avatar.animation
{
   ...
   public class Animation implements IAnimation
   {
      ...
      private var var_1400:AvatarDataContainer;
      
      private var _frames:Array;
      
      private var var_1399:DirectionDataContainer;
      
      private var var_1401:ShadowDataContainer;
      ...
      public function Animation(param1:AvatarStructure, param2:XML)
      {
         ...
         if(param2.hasOwnProperty("sprite"))
         {
            ...
            {
               var_1132.push(new SpriteDataContainer(this as IAnimation,_loc5_));
            }
         }
         if(param2.hasOwnProperty("avatar"))
         {
            var_1400 = new AvatarDataContainer(param2.avatar[0]);
         }
         if(param2.hasOwnProperty("canvas"))
         {
            for each(_loc6_ in param2.canvas)
            {
               var_1133.push(new CanvasDataContainer(_loc6_));
            }
         }
         if(param2.hasOwnProperty("direction"))
         {
            var_1399 = new DirectionDataContainer(param2.direction[0]);
         }
         if(param2.hasOwnProperty("shadow"))
         {
            var_1401 = new ShadowDataContainer(param2.shadow[0]);
         }
         if(param2.hasOwnProperty("remove"))
         {
            for each(_loc7_ in param2.remove)
            {
               var_1131.push(String(_loc7_.@id));
            }
         }
         if(param2.hasOwnProperty("add"))
         {
            ...
            {
               var_645.push(new AddDataContainer(_loc8_));
            }
         }
         var _loc3_:int = 0;
         for each(_loc4_ in param2.frame)
         {
            ...
            for each(_loc12_ in _loc4_.fx)
            {
               ...
               _loc14_ = new AnimationLayerData(param1,_loc12_,AnimationLayerData.const_362,_loc3_,_loc10_);
               _loc9_.push(_loc14_);
               if(_loc10_ != null)
               {
               }
      ...
      public function getLayerData(param1:int, param2:String) : AnimationLayerData
      {
         ...
         for each(_loc3_ in getFrame(param1))
         {
            ...
            if(_loc3_.type == AnimationLayerData.const_362)
            {
               for each(_loc4_ in var_645)
               {
