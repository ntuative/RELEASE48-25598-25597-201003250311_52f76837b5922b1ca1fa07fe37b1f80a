package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      private var var_478:IItemGridWindow;
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(_view != null)
         {
            var_478 = _view.findChildByName("part_thumbs") as IItemGridWindow;
            var_400 = new Array();
            var_400.push(_view.findChildByName("palette0") as IItemGridWindow);
            var_400.push(_view.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_49)
         {
            ...
            _loc7_ = 0;
            while(_loc7_ < const_1026)
            {
               if(var_400.length > _loc7_)
               {
                  _loc4_ = var_400[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     _model.selectColor(var_1154,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = _model.getCategoryData(var_1154);
         if(!_loc1_)
         {
            return;
         }
         var_478.removeGridItems();
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               var_478.addGridItem(_loc2_.view);
         ...
         _loc3_ = 0;
         while(_loc3_ < const_1026)
         {
            _loc4_ = _loc1_.getPalette(_loc3_);
            _loc5_ = var_400[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = var_478.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_49)
         {
            _loc3_ = var_478.getGridItemIndex(param1.window);
            _model.selectPart(var_1154,_loc3_);
         }
      }
